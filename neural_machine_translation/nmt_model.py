...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder = nn.LSTM(embed_size, hidden_size, bidirectional=True)  # Default bias=True.
        self.decoder = nn.LSTMCell(embed_size + hidden_size, hidden_size)  # Default bias=True.
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, 1):
            Y_t = torch.squeeze(Y_t, dim=0)  # Squeeze along dim=0.
            Ybar_t = torch.cat((Y_t, o_prev), dim=1)
            dec_state, o_t, e_t = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
            o_prev = o_t  # Prepare for the next iteration.
            combined_outputs.append(o_t)  # Append to our result.
